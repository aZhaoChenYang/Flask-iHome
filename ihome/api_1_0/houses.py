...
# GET /api/v1.0/houses?sd=2020-12-01&ed=2020-12-31&aid=10&sk=new&p=1
@api.route("/houses")
def get_house_list():
    """获取房屋的列表信息(搜索页面)"""
    ...
    if conflict_orders:
        ...
        # filter_params = [House.id.notin_(conflict_house_ids)]
    ...
    try:
        # 处理分页
        #                               当前页数        每页数据量                                               自动的错误输出
        page_obj = house_query.paginate(page=page, per_page=constants.HOUES_LIST_PAGE_REDIS_CACHE_EXPIRES, error_out=False)
    except Exception as e:
        current_app.logger.error(e)
        return jsonify(errno = RET.DBERR, errmsg = "数据库异常")
